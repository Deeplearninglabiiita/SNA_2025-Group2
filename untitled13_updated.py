...
def train_model(data):
    ...
    # Evaluate model (classification report removed)
    y_pred = model.predict(X_test)
    cm = confusion_matrix(y_pred, y_test, labels=le.transform(le.classes_))
    ...
    return model, le, scaler, feature_importance, cm
...
def main():
    ...
    page = st.sidebar.radio("Go to", ["Dataset Overview", "Data Input & Prediction", "Model Visualizations"])
    ...
    if os.path.exists('hoax_call_detector.pkl'):
        ...
        data = assign_classes(data)
        data, G = social_network_features(data)
        model, le, scaler, feature_importance, cm = train_model(data)
    ...
    if page == "Dataset Overview":
        ...
        if os.path.exists('confusion_matrix.npy'):
            ...
            sns.heatmap(cm, annot=True, fmt='d', cmap='Blues',
                       xticklabels=['Business', 'Family', 'Other', 'Regular', 'Scam', 'Spam'],
                       yticklabels=['Business', 'Family', 'Other', 'Regular', 'Scam', 'Spam'])
            plt.ylabel('Actual')
